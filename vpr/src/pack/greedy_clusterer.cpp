...
GreedyClusterer::do_clustering(ClusterLegalizer& cluster_legalizer,
                               ...
                               DeviceContext& mutable_device_ctx, vtr::vector<AtomBlockId, float>& atom_criticality) {
    ...
    // since atom_criticality is passed by reference, we initalize its values direct
    atom_criticality = vtr::vector<AtomBlockId, float>(atom_netlist_.blocks().size(), 0.f);
