...
class PlacementAnnealer {
  public:
     ...
     */
    bool placement_inner_loop(float layer_weight=1.0, float timing_tradeoff=0.5);
     ...
     */
    void outer_loop_update_timing_info(float layer_weight = 1.0f, float timing_tradeoff = 0.5f);
    ...
    /**
     * @brief Kick move to help the annealer escape local minima.
     *
     * This function performs a kick move, which is a random swap of blocks
     * to help the annealer escape local minima. The percentage of blocks to swap
     * is specified by the `percent_to_swap` parameter.
     *
     * @param percent_to_swap Percentage of blocks to swap in the kick move.
     */
    void lsmc_kick_move(float percent_to_swap, float layer_weight =1.0f);

    /**
     * @brief updates the bb cost and timing cost after restoring best placement post LSMC kick move
     */
    void update_costs_after_kick_move(float layer_weight = 1.0f, float timing_tradeoff = 0.5f);
    ...
    /**
     * @brief Updates the agent state to either EARLY_IN_THE_ANNEAL or LATE_IN_THE_ANNEAL.
     * @param agent_state The new agent state to set.
     * 
     */
    void set_agent_state(e_agent_state agent_state);
  ...
  private:
    ...
    e_move_result try_swap_(MoveGenerator& move_generator,
                            ...
                            bool manual_move_enabled, float layer_weight = 1.0, float timing_tradeoff = 0.5);
