...
bool find_to_loc_uniform(t_logical_block_type_ptr type,
    ...
    const int to_layer_num = stuck_layer ? from.layer : get_random_layer(type, rng);
...
bool find_to_loc_centroid(t_logical_block_type_ptr blk_type,
    ...
    //Retrieve the compressed block grid for this block type
    const auto& compressed_block_grid = g_vpr_ctx.placement().compressed_block_grids[blk_type->index];
    int to_layer_num = centroid.layer;
    ...
    // if the centroid location for from_layer is invalid, round to the next nearest layer somehow
    if (centroid_compressed_loc[to_layer_num].layer_num == OPEN){
        // find nearest valid layer
        int nearest_layer = -1;
        int nearest_layer_distance = std::numeric_limits<int>::max();
        for (int ilayer = 0; ilayer < num_layers; ilayer++){
            if (ilayer == to_layer_num) continue; // skip the original invalid layer
            if (centroid_compressed_loc[ilayer].layer_num != OPEN){
                int layer_distance = std::abs(ilayer - to_layer_num);
                if (layer_distance < nearest_layer_distance){
                    nearest_layer_distance = layer_distance;
                    nearest_layer = ilayer;
                }
            }
        }

        to_layer_num = nearest_layer;
        VTR_ASSERT(to_layer_num != OPEN);
    }
...
static const std::array<std::string, (int) e_move_type::INVALID_MOVE> move_type_strings = {
    "Uniform",
    "Median",
    "Centroid",
    "W. Centroid",
    ...
    "W. Centroid Prob.",
    "Median Prob.",
    "Centroid Layer Stuck",
    "W. Centroid Layer Stuck",
    "Median Layer Stuck",
    "Uniform Layer Stuck",
    "Two Opt",
    "W. Median Prob.",
    "W. Median Layer Stuck",
    "NoC Centroid",
    "Manual Move"
};
...
bool find_compatible_compressed_loc_in_range(t_logical_block_type_ptr type,
    ...
    while (!legal && (int)tried_cx_to.size() < possibilities) { //Until legal or all possibilities exhausted
        ...
        if (block_rows.empty()) {
            VTR_LOG("\tNo valid blocks at x=%d, layer=%d\n", to_loc.x, to_layer_num);
            continue; //No valid blocks at this x location
        }
