...
double t_placer_costs::get_total_cost(const t_placer_opts& placer_opts, const t_noc_opts& noc_opts, float timing_tradeoff) {
    ...
    if (placer_opts.place_algorithm == e_place_algorithm::BOUNDING_BOX_PLACE) {
        ...
        total_cost = (1 - timing_tradeoff) * (bb_cost * bb_cost_norm) + (timing_tradeoff) * (timing_cost * timing_cost_norm);
    }
