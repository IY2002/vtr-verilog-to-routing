    ...
    for (const RRNodeId src_rr_node : rr_graph.nodes()) {
        for (const t_edge_size rr_edge_idx : rr_graph.edges(src_rr_node)) {
            ...
            if (rr_graph.node_layer(src_rr_node) != rr_graph.node_layer(sink_rr_node)) {
                ...
                // There are 3 scenarios where the inter-die connection is made:
                // 1. a 3D output pin is connected to a channel on the other layer
                //      in this case, the source is the pin (whose xlow and ylow are the same as xhigh and yhigh), and the sink is the channel.
                //      The location of the interlayer connection is then that of the source node
                // 2. a channel on the other layer is connected to a 3D input pin
                //      in this case, the source is the channel (whose xlow and ylow are not necessarily the same as xhigh and yhigh, multi-length segments), and the sink is the pin (whose xlow and ylow are the same as xhigh and yhigh).
                //      The location of the interlayer connection is then that of the sink node then since we know it's location for certain
                // 3. a channel is connected to a channel on a different layer (3D SB)
                //      in this case, since it only happens with 3D SB who have the special CHANZ node that don't have a direction and have the same xlow and xhigh either the sink or source node can be used. 

                if (rr_graph.node_xlow(src_rr_node) == src_x && rr_graph.node_ylow(src_rr_node) == src_y){
                    tile_num_inter_die_conn[src_x][src_y]++;
                } else{
                    int sink_x = rr_graph.node_xhigh(sink_rr_node);
                    int sink_y = rr_graph.node_yhigh(sink_rr_node);
                    VTR_ASSERT(rr_graph.node_xlow(sink_rr_node) == sink_x && rr_graph.node_ylow(sink_rr_node) == sink_y);
                    tile_num_inter_die_conn[sink_x][sink_y]++;
                }
...
void NetCostHandler::update_td_delta_costs_(const PlaceDelayModel* delay_model,
                                            ...
                                            bool is_src_moving, float layer_weight) {
    /**
    ...
    if (cluster_ctx.clb_nlist.pin_type(pin) == PinType::DRIVER) {
        ...
        for (size_t ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net).size(); ipin++) {
            float temp_delay = comp_td_single_connection_delay(delay_model, block_locs, net, ipin, layer_weight);
...
double NetCostHandler::get_net_per_layer_bb_cost_(ClusterNetId net_id , bool use_ts) {
    ...
    for (int layer_num = 0; layer_num < num_layers; layer_num++) {
        ...
        double layer_ncost = 0.;
    //     ncost += (bb[layer_num].xmax - bb[layer_num].xmin + 1) * chanx_cost_fac;
    //     ncost += (bb[layer_num].ymax - bb[layer_num].ymin + 1) * chany_cost_fac;
    //     ncost *= crossing;
    
        layer_ncost = (bb[layer_num].xmax - bb[layer_num].xmin + 1) * chanx_cost_fac;
        layer_ncost += (bb[layer_num].ymax - bb[layer_num].ymin + 1) * chany_cost_fac;
        layer_ncost *= crossing;
        ncost += layer_ncost;
    }
...
void NetCostHandler::find_affected_nets_and_update_costs(const PlaceDelayModel* delay_model,
                                                         ...
                                                         double& bb_delta_c,
                                                         double& timing_delta_c, float layer_weight) {
...
void NetCostHandler::recompute_costs_from_scratch(const PlaceDelayModel* delay_model,
                                                  const PlacerCriticalities* criticalities,
                                                  t_placer_costs& costs, float layer_weight) {
    ...
    if (placer_opts_.place_algorithm.is_timing_driven()) {
        ...
        comp_td_costs(delay_model, *criticalities, placer_state_, &new_timing_cost, layer_weight);
