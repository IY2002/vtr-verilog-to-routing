...
CentroidLayerStuckMoveGenerator::CentroidLayerStuckMoveGenerator(PlacerState& placer_state,
...
e_create_move CentroidLayerStuckMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
...
const std::vector<ClusterBlockId>& CentroidLayerStuckMoveGenerator::get_noc_group_routers(NocGroupId noc_grp_id) {
    return noc_group_routers_[noc_grp_id];
}

NocGroupId CentroidLayerStuckMoveGenerator::get_cluster_noc_group(ClusterBlockId blk_id) {
    return cluster_to_noc_grp_[blk_id];
}

void CentroidLayerStuckMoveGenerator::initialize_noc_groups(size_t high_fanout_net) {
    const auto& cluster_ctx = g_vpr_ctx.clustering();
...
t_pl_loc CentroidLayerStuckMoveGenerator::calculate_centroid_loc_(ClusterBlockId b_from,
                                                        const PlacerCriticalities* criticalities) {
    const auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    if (noc_attraction_enabled_) {
        NocGroupId noc_grp_id = CentroidLayerStuckMoveGenerator::get_cluster_noc_group(b_from);

        // check if the block belongs to a NoC group
        if (noc_grp_id != NocGroupId::INVALID()) {
            // get the routers in the associated NoC group
            const auto& noc_routers = CentroidLayerStuckMoveGenerator::get_noc_group_routers(noc_grp_id);
    ...
    centroid.layer = from_block_layer_num; // layer stuck
