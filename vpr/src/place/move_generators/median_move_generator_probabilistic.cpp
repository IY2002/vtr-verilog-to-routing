...
MedianProbMoveGenerator::MedianProbMoveGenerator(PlacerState& placer_state,
                                         e_reward_function reward_function,
                                         vtr::RngContainer& rng)
    : MoveGenerator(placer_state, reward_function, rng) {}

e_create_move MedianProbMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
    ...
    for (ClusterPinId pin_id : cluster_ctx.clb_nlist.block_pins(b_from)) {
        ...
        // Track layer frequency for probabilistic selection
        layer_frequency[coords.layer_min]++;
        layer_frequency[coords.layer_max]++;
    }
    ...
    // Still calculate these for fallback/range limiting
    ...
    if (!layer_frequency.empty()) {
        ...
        if (!cumulative_dist.empty() && cumulative_freq > 0) {
            ...
            median_point.layer = limit_coords.layer_min; // default fallback
            for (const auto& [layer, cum_freq] : cumulative_dist) {
                if (random_val <= cum_freq) {
                    median_point.layer = layer;
                    break;
                }
            }
        } else {
...
void MedianProbMoveGenerator::get_bb_from_scratch_excluding_block(ClusterNetId net_id,
                                                              t_bb& bb_coord_new,
...
bool MedianProbMoveGenerator::get_bb_incrementally(ClusterNetId net_id,
                                               t_bb& bb_coord_new,
