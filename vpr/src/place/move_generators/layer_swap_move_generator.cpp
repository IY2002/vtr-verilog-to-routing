...
LayerSwapMoveGenerator::LayerSwapMoveGenerator(PlacerState& placer_state,
                                               e_reward_function reward_function,
                                               vtr::RngContainer& rng)
    : MoveGenerator(placer_state, reward_function, rng) {}

e_create_move LayerSwapMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
    ...
    // Randomly select layers to try
    bool found_valid_layer = false;
    t_pl_loc to = from; // Start with same location
    
    while (!candidate_layers.empty() && !found_valid_layer) {
        ...
        // Check if this layer has a compatible tile at the same X,Y location
        if (to.x >= 0 && to.x < int(device_ctx.grid.width()) &&
            ...
            // Check if the tile type is compatible with the block type
            if (is_tile_compatible(grid_to_type, cluster_from_type)) {
                found_valid_layer = true;
                // The subtile will be handled by create_move
            }
        }
    }
    ...
    if (!found_valid_layer) {
        VTR_LOGV_DEBUG(g_vpr_ctx.placement().f_placer_debug, 
                       "\tNo valid layer found for swap at location (%d,%d)\n", 
                       from.x, from.y);
        return e_create_move::ABORT;
    }

    VTR_LOGV_DEBUG(g_vpr_ctx.placement().f_placer_debug, 
                   "\tSwapping block from layer %d to layer %d at (%d,%d)\n", 
                   from.layer, to.layer, from.x, from.y);

    // Create the move - this will handle subtile assignment internally
