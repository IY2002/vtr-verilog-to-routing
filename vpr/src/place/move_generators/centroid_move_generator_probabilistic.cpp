...
CentroidProbMoveGenerator::CentroidProbMoveGenerator(PlacerState& placer_state,
...
e_create_move CentroidProbMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
...
const std::vector<ClusterBlockId>& CentroidProbMoveGenerator::get_noc_group_routers(NocGroupId noc_grp_id) {
    return noc_group_routers_[noc_grp_id];
}

NocGroupId CentroidProbMoveGenerator::get_cluster_noc_group(ClusterBlockId blk_id) {
    return cluster_to_noc_grp_[blk_id];
}

void CentroidProbMoveGenerator::initialize_noc_groups(size_t high_fanout_net) {
    const auto& cluster_ctx = g_vpr_ctx.clustering();
...
t_pl_loc CentroidProbMoveGenerator::calculate_centroid_loc_(ClusterBlockId b_from,
                                                        const PlacerCriticalities* criticalities) {
    const auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    // Instead of accumulating a single layer value, track weights per layer
    std::map<int, float> layer_weights;
    ...
    for (ClusterPinId pin_id : cluster_ctx.clb_nlist.block_pins(b_from)) {
        ...
        if (cluster_ctx.clb_nlist.pin_type(pin_id) == PinType::DRIVER) {
            ...
            for (auto sink_pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
                ...
                // Accumulate weight for this specific layer
                layer_weights[tile_loc.layer_num] += weight;
    ...
    if (noc_attraction_enabled_) {
        NocGroupId noc_grp_id = CentroidProbMoveGenerator::get_cluster_noc_group(b_from);

        // check if the block belongs to a NoC group
        if (noc_grp_id != NocGroupId::INVALID()) {
            // get the routers in the associated NoC group
            const auto& noc_routers = CentroidProbMoveGenerator::get_noc_group_routers(noc_grp_id);
            ...
            // Scale existing layer weights
            for (auto& [layer, layer_weight] : layer_weights) {
                layer_weight *= (1.0f - noc_attraction_weight_);
            }
            ...
            for (ClusterBlockId router_blk_id : noc_routers) {
                ...
                // Add NoC router layer weight
                layer_weights[router_loc.loc.layer] += single_noc_weight;
    ...
    t_pl_loc centroid;
    //Calculate the deterministic centroid location for x and y
    centroid.x = (int)std::round(acc_x / acc_weight);
    centroid.y = (int)std::round(acc_y / acc_weight);
    
    // Probabilistically select layer based on weights
    if (!layer_weights.empty()) {
        // Create cumulative distribution
        std::vector<std::pair<int, float>> cumulative_dist;
        float cumulative_weight = 0.0f;
        
        for (const auto& [layer, weight] : layer_weights) {
            cumulative_weight += weight;
            cumulative_dist.push_back({layer, cumulative_weight});
        }
        
        // Generate random number between 0 and total weight
        float random_val = rng_.frand() * cumulative_weight;

        // Find which layer this random value falls into
        for (const auto& [layer, cum_weight] : cumulative_dist) {
            if (random_val <= cum_weight) {
                centroid.layer = layer;
                break;
            }
        }
    } else {
        // Fallback to from_block_layer_num if no valid layers found
