...
UniformLayerStuckMoveGenerator::UniformLayerStuckMoveGenerator(PlacerState& placer_state,
                                           e_reward_function reward_function,
                                           vtr::RngContainer& rng)
    : MoveGenerator(placer_state, reward_function, rng) {}

e_create_move UniformLayerStuckMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
    ...
    t_pl_loc to;
    if (!find_to_loc_uniform(cluster_from_type, rlim, from, to, b_from, blk_loc_registry, rng_, true)) {
        return e_create_move::ABORT;
    }

    // // keep the layer the same hence stuck
    // to.layer = from.layer;
