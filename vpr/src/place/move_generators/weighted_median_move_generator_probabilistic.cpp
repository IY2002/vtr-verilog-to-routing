#include "weighted_median_move_generator_probabilistic.h"
...
WeightedMedianProbMoveGenerator::WeightedMedianProbMoveGenerator(PlacerState& placer_state,
                                                         e_reward_function reward_function,
                                                         vtr::RngContainer& rng)
    : MoveGenerator(placer_state, reward_function, rng) {}

e_create_move WeightedMedianProbMoveGenerator::propose_move(t_pl_blocks_to_be_moved& blocks_affected,
    ...
    // Map to track layer frequency/weights for probabilistic selection
    std::map<int, int> layer_frequency;
    ...
    for (ClusterPinId pin_id : cluster_ctx.clb_nlist.block_pins(b_from)) {
        ...
        // Update layer frequency map
        // Track layer frequency for probabilistic selection based on weight
        int layer_min_weight = static_cast<int>(std::ceil(coords.layer_min.criticality * CRIT_MULT_FOR_W_MEDIAN));
        if (layer_min_weight > 0)
            layer_frequency[coords.layer_min.edge] += layer_min_weight;

        int layer_max_weight = static_cast<int>(std::ceil(coords.layer_max.criticality * CRIT_MULT_FOR_W_MEDIAN));
        if (layer_max_weight > 0)
            layer_frequency[coords.layer_max.edge] += layer_max_weight;
    }
    ...
    // size_t n = place_move_ctx.X_coord.size();
    // size_t median_idx = (n - 1) / 2;
    // size_t median_idx_plus_one = median_idx + 1;

    // // X coordinates
    // if (median_idx_plus_one < n) {
    //     std::nth_element(place_move_ctx.X_coord.begin(), 
    //                     place_move_ctx.X_coord.begin() + median_idx_plus_one, 
    //                     place_move_ctx.X_coord.end());
    //     limit_coords.xmax = place_move_ctx.X_coord[median_idx_plus_one];
    //     limit_coords.xmin = *std::max_element(place_move_ctx.X_coord.begin(), 
    //                                         place_move_ctx.X_coord.begin() + median_idx_plus_one);
    // } else {
    //     std::nth_element(place_move_ctx.X_coord.begin(), 
    //                     place_move_ctx.X_coord.begin() + median_idx, 
    //                     place_move_ctx.X_coord.end());
    //     limit_coords.xmin = limit_coords.xmax = place_move_ctx.X_coord[median_idx];
    // }

    // // Y coordinates  
    // n = place_move_ctx.Y_coord.size();
    // median_idx = (n - 1) / 2;
    // median_idx_plus_one = median_idx + 1;

    // if (median_idx_plus_one < n) {
    //     std::nth_element(place_move_ctx.Y_coord.begin(), 
    //                     place_move_ctx.Y_coord.begin() + median_idx_plus_one, 
    //                     place_move_ctx.Y_coord.end());
    //     limit_coords.ymax = place_move_ctx.Y_coord[median_idx_plus_one];
    //     limit_coords.ymin = *std::max_element(place_move_ctx.Y_coord.begin(), 
    //                                         place_move_ctx.Y_coord.begin() + median_idx_plus_one);
    // } else {
    //     std::nth_element(place_move_ctx.Y_coord.begin(), 
    //                     place_move_ctx.Y_coord.begin() + median_idx, 
    //                     place_move_ctx.Y_coord.end());
    //     limit_coords.ymin = limit_coords.ymax = place_move_ctx.Y_coord[median_idx];
    // }

    // // Layer coordinates
    // n = place_move_ctx.layer_coord.size();
    // median_idx = (n - 1) / 2;
    // median_idx_plus_one = median_idx + 1;

    // if (median_idx_plus_one < n) {
    //     std::nth_element(place_move_ctx.layer_coord.begin(), 
    //                     place_move_ctx.layer_coord.begin() + median_idx_plus_one, 
    //                     place_move_ctx.layer_coord.end());
    //     limit_coords.layer_max = place_move_ctx.layer_coord[median_idx_plus_one];
    //     limit_coords.layer_min = *std::max_element(place_move_ctx.layer_coord.begin(), 
    //                                             place_move_ctx.layer_coord.begin() + median_idx_plus_one);
    // } else {
    //     std::nth_element(place_move_ctx.layer_coord.begin(), 
    //                     place_move_ctx.layer_coord.begin() + median_idx, 
    //                     place_move_ctx.layer_coord.end());
    //     limit_coords.layer_min = limit_coords.layer_max = place_move_ctx.layer_coord[median_idx];
    // }

    t_pl_loc w_median_point;
    // Probabilistic layer selection instead of deterministic median
    if (!layer_frequency.empty()) {
        // Create cumulative distribution from layer frequencies
        std::vector<std::pair<int, int>> cumulative_dist;
        int cumulative_freq = 0;
        
        // Filter layers to only those within the median range
        for (const auto& [layer, freq] : layer_frequency) {
            if (layer >= limit_coords.layer_min && layer <= limit_coords.layer_max) {
                cumulative_freq += freq;
                cumulative_dist.push_back({layer, cumulative_freq});
            }
        }
        
        // If we have valid layers in range, select probabilistically
        if (!cumulative_dist.empty() && cumulative_freq > 0) {
            // Generate random number between 0 and total frequency
            float random_val = rng_.frand() * cumulative_freq;
            
            // Find which layer this random value falls into
            w_median_point.layer = limit_coords.layer_min; // default fallback
            for (const auto& [layer, cum_freq] : cumulative_dist) {
                if (random_val <= cum_freq) {
                    w_median_point.layer = layer;
                    break;
                }
            }
        } else {
            // Fallback to deterministic median if no valid layers in range
            w_median_point.layer = (limit_coords.layer_min + limit_coords.layer_max) / 2;
        }
    } else {
        // Fallback to deterministic median if no layer data
        w_median_point.layer = (limit_coords.layer_min + limit_coords.layer_max) / 2;
    }
   ...
   std::stable_sort(place_move_ctx.layer_coord.begin(), place_move_ctx.layer_coord.end());
    ...
    // Keep the same layer as the source block
    w_median_point.layer = from.layer; // Keep the same layer as the source block
...
bool WeightedMedianProbMoveGenerator::get_bb_cost_for_net_excluding_block(ClusterNetId net_id,
