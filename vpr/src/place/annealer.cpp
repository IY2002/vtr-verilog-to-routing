...
e_move_result PlacementAnnealer::try_swap_(MoveGenerator& move_generator,
                                           ...
                                           bool manual_move_enabled, float layer_weight, float timing_tradeoff) {
    ...
    if (create_move_outcome == e_create_move::ABORT) {
        ...
        net_cost_handler_.find_affected_nets_and_update_costs(delay_model_, criticalities_, blocks_affected_,
                                                              bb_delta_c, timing_delta_c, layer_weight);
        ...
        if (place_algorithm == e_place_algorithm::CRITICALITY_TIMING_PLACE) {
            ...
            if (placer_opts_.timing_tradeoff_adjustor == e_timing_tradeoff_adjustor::NONE) {
                ...
                timing_tradeoff = placer_opts_.timing_tradeoff;
            } 
        ...
        if (move_outcome == e_move_result::ACCEPTED) {
            ...
            if (place_algorithm == e_place_algorithm::CRITICALITY_TIMING_PLACE) {
                ...
                comp_td_connection_delays(delay_model_, placer_state_, layer_weight);
                comp_td_costs(delay_model_, *criticalities_, placer_state_, &costs_.timing_cost, layer_weight);
...
void PlacementAnnealer::outer_loop_update_timing_info(float layer_weight, float timing_tradeoff) {
    if (placer_opts_.place_algorithm.is_timing_driven()) {
        ...
        if (outer_crit_iter_count_ >= placer_opts_.recompute_crit_iter ||
            ...
            perform_full_timing_update(crit_params, delay_model_, criticalities_, setup_slacks_,
                                       pin_timing_invalidator_, timing_info_, &costs_, placer_state_, layer_weight);
    ...
    costs_.cost = costs_.get_total_cost(placer_opts_, noc_opts_, timing_tradeoff);
}

void PlacementAnnealer::update_costs_after_kick_move(float layer_weight, float timing_tradeoff) {
    // Recalculate the bounding box cost
    costs_.bb_cost = net_cost_handler_.comp_bb_cost(e_cost_methods::CHECK);

    // For timing-driven placement, recalculate timing costs from scratch
    if (placer_opts_.place_algorithm.is_timing_driven()) {
        // Recompute all connection delays
        comp_td_connection_delays(delay_model_, placer_state_, layer_weight);

        // Recompute timing costs
        comp_td_costs(delay_model_, *criticalities_, placer_state_, &costs_.timing_cost, layer_weight);

        // Do a full timing update
        PlaceCritParams crit_params{annealing_state_.crit_exponent,
                                     placer_opts_.place_crit_limit};
        perform_full_timing_update(crit_params, delay_model_, criticalities_, setup_slacks_,
                                    pin_timing_invalidator_, timing_info_, &costs_, placer_state_, layer_weight);
    ...
    // Update the cost normalization factors and total cost
    costs_.update_norm_factors();
    costs_.cost = costs_.get_total_cost(placer_opts_, noc_opts_, timing_tradeoff);
}

float compute_block_criticality(ClusterBlockId blk_id,
    ...
    for (ClusterPinId pin_id : cluster_ctx.clb_nlist.block_pins(blk_id)) {
        ...
        // Ignore degenerate self-loop nets
        ...
        // Driver pin -> check sinks
        if (cluster_ctx.clb_nlist.pin_type(pin_id) == PinType::DRIVER) {
            for (auto sink_pin_id : cluster_ctx.clb_nlist.net_sinks(net_id)) {
                if (pin_id == sink_pin_id) continue; // skip trivial
                int ipin = cluster_ctx.clb_nlist.pin_net_index(sink_pin_id);
                float weight = (criticalities) ? criticalities->criticality(net_id, ipin) : 1.0f;
                acc_weight += weight;
            }
        } 
        // Sink pin -> check driver
        else {
...
int num_kick_moves = 0;

/**
 * @brief Perform a kick move for the LSMC algorithm.
 *
 * This function is a placeholder for the LSMC kick move logic.
 * It currently does not implement any functionality.
 *
 * @param percent_to_swap The percentage of blocks to swap in the kick move.
 */
void PlacementAnnealer::lsmc_kick_move(float percent_to_swap, float layer_weight) {
    const auto& cluster_ctx = g_vpr_ctx.clustering();
    const auto& device_ctx = g_vpr_ctx.device();
    auto& blk_loc_registry = placer_state_.mutable_blk_loc_registry();
    
    // Calculate number of blocks to swap
    size_t num_blocks = cluster_ctx.clb_nlist.blocks().size();
    size_t num_blocks_to_swap = static_cast<size_t>(num_blocks * percent_to_swap / 100.0);
    
    VTR_LOG("LSMC kick move: attempting to swap %zu blocks (%.1f%% of %zu total blocks)\n", 
            num_blocks_to_swap, percent_to_swap, num_blocks);
    
    size_t successful_swaps = 0;
    
    // Vector of (block, criticality)
    std::vector<std::pair<ClusterBlockId, float>> block_scores;

    for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
        if (!blk_id) continue;

        float score = compute_block_criticality(blk_id, cluster_ctx, criticalities_, blk_loc_registry);
        block_scores.emplace_back(blk_id, score);
    }

    size_t num_to_swap = std::min(num_blocks_to_swap, block_scores.size());
    ...
    std::nth_element(block_scores.begin(), block_scores.begin() + num_to_swap, block_scores.end(),
    [](auto& a, auto& b) { return a.second > b.second; });

    // Only sort top-N
    std::sort(block_scores.begin(), block_scores.begin() + num_to_swap,
        [](auto& a, auto& b) { return a.second > b.second; });

    
    // // Create a vector of all block IDs
    // std::vector<ClusterBlockId> all_blocks;
    // for (auto blk_id : cluster_ctx.clb_nlist.blocks()) {
    //     if (blk_id) all_blocks.push_back(blk_id);
    // }
    
    // // Manually shuffle using VTR's RNG
    // for (size_t i = all_blocks.size() - 1; i > 0; i--) {
    //     size_t j = rng_.irand(i);
    //     std::swap(all_blocks[i], all_blocks[j]);
    // }

    
    // Process blocks in random order
    // for (size_t i = 0; i < all_blocks.size() && successful_swaps < num_blocks_to_swap; i++) {
    for (size_t i = 0; i < num_to_swap && successful_swaps < num_blocks_to_swap; i++) {
        // ClusterBlockId b_from = all_blocks[i];
        ClusterBlockId b_from = block_scores[i].first;

        // Clear previous move data
        blocks_affected_.clear_move_blocks();
        
        // Get block info
        t_pl_loc from = blk_loc_registry.block_locs()[b_from].loc;
        auto cluster_from_type = cluster_ctx.clb_nlist.block_type(b_from);
        
        // Use find_to_loc_uniform
        t_pl_loc to;
        if (!find_to_loc_uniform(cluster_from_type, 
                                (device_ctx.grid.width() - 1) * placer_opts_.kick_move_swap_range_ratio,
                                from, 
                                to, 
                                b_from, 
                                blk_loc_registry, 
        ...
        // Create the move
        e_create_move create_move_result = ::create_move(blocks_affected_, b_from, to, blk_loc_registry);
        
        if (create_move_result != e_create_move::VALID) {
            continue;
        }
        ...
        // Find affected nets and update costs (even though we're not using them now)
        // This is important to keep the net_cost_handler's internal state consistent
        double bb_delta_c = 0;
        double timing_delta_c = 0;
        net_cost_handler_.find_affected_nets_and_update_costs(delay_model_, criticalities_, 
                                                              blocks_affected_,
                                                              bb_delta_c, timing_delta_c);
        

        // Update net cost functions and reset flags (like accepted move in try_swap_)
        net_cost_handler_.update_move_nets();
        ...
        VTR_LOGV_DEBUG("LSMC kick move: Swapped block %s with id %d from (%d, %d) to (%d, %d)\n", 
                cluster_ctx.clb_nlist.block_name(b_from).c_str(),
                (int) b_from, from.x, from.y, to.x, to.y);
    }

    // Clear previous move data
    blocks_affected_.clear_move_blocks();
    
    // After all moves, recalculate all costs from scratch
    update_costs_after_kick_move(layer_weight);
    
    // Increase temp by 10x and rlim to be half the grid size
    annealing_state_.t *= placer_opts_.kick_move_temp_inc_factor;
    float rlim_increase_factor = placer_opts_.kick_move_rlim_inc_ratio;
    annealing_state_.rlim = (float)std::max(device_ctx.grid.width() * rlim_increase_factor, device_ctx.grid.height() * rlim_increase_factor);

    VTR_LOG("LSMC kick move completed: %zu successful swaps. New bb_cost: %.2f, timing_cost: %.4e\n", 
            successful_swaps, costs_.bb_cost, costs_.timing_cost);
}

bool PlacementAnnealer::placement_inner_loop(float layer_weight, float timing_tradeoff) {
    ...
    for (int inner_iter = 0, inner_crit_iter_count = 1; inner_iter < annealing_state_.move_lim; inner_iter++) {
        e_move_result swap_result = try_swap_(move_generator, placer_opts_.place_algorithm, manual_move_enabled, layer_weight, timing_tradeoff);
        ...
        if (placer_opts_.place_algorithm.is_timing_driven()) {
            ...
            if (inner_crit_iter_count >= recompute_limit && inner_iter != annealing_state_.move_lim - 1) {
                ...
                perform_full_timing_update(crit_params, delay_model_, criticalities_,
                                           ...
                                           timing_info_, &costs_, placer_state_, layer_weight);
            }
        ...
        if (moves_since_cost_recompute_ > MAX_MOVES_BEFORE_RECOMPUTE) {
            net_cost_handler_.recompute_costs_from_scratch(delay_model_, criticalities_, costs_, layer_weight);
    ...
    if (!quench_started_) {
        ...
        agent_state_ == e_agent_state::EARLY_IN_THE_ANNEAL) {
            if (annealing_state_.alpha < 0.85 && annealing_state_.alpha > 0.6) {
                // if (num_kick_moves < placer_opts_.kick_move_num && placer_opts_.enable_kick_move){
                //     num_kick_moves++;
                //     VTR_LOG("LSMC kick move %d\n", num_kick_moves);
                //     // ideally we want to checkpoint the placement here before the kick move
                //     // but we don't have a way to do that yet, so we just log the kick for now
                //     // need to pass down the things needed for checkpointing
                //     // lsmc_kick_move(50.0);
                //     kick_move = true;
                // }
                // else{
...
void PlacementAnnealer::set_agent_state(e_agent_state agent_state) {
    agent_state_ = agent_state;
    if (agent_state == e_agent_state::EARLY_IN_THE_ANNEAL) {
        VTR_LOG("Agent's 1st state: EARLY_IN_THE_ANNEAL\n");
    } else if (agent_state == e_agent_state::LATE_IN_THE_ANNEAL) {
        VTR_LOG("Agent's 2nd state: LATE_IN_THE_ANNEAL\n");
    } else {
        VTR_ASSERT_MSG(false, "Unknown agent state");
    }
}